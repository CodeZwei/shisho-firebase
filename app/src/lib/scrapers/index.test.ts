...
import { scraper as rule34HtmlScraper } from './rule34-html.js';
import { scraper as genericScraper } from './generic.js';

vi.mock('./rule34-html.js', () => ({
	scraper: {
		name: 'rule34-html',
...
describe('scrape dispatcher', () => {
	beforeEach(() => {
		vi.mocked(rule34HtmlScraper.scrape).mockResolvedValue({ external: { title: 'rule34 post' } });
	...
	it('routes rule34.xxx to the rule34-html scraper', async () => {
		const result = await scrape(RULE34_URL);

		expect(vi.mocked(rule34HtmlScraper.scrape)).toHaveBeenCalledWith(RULE34_URL, undefined);
		expect(vi.mocked(genericScraper.scrape)).not.toHaveBeenCalled();
		expect(result).toEqual({ external: { title: 'rule34 post' }, parser: 'rule34-html' });
	});
	...
	it('routes unknown hostnames to the generic scraper', async () => {
		...
		expect(vi.mocked(rule34HtmlScraper.scrape)).not.toHaveBeenCalled();
